...
- (id) initWithBoundary:(NSString*) boundary formEncoding:(NSStringEncoding) _formEncoding {
	...
	if( nil == boundary ) {
		DDLogWarn(@"MultipartFormDataParser: init with zero boundary");
		return nil;
	}
...
- (BOOL) appendData:(NSData *)data { 
    ...
    if( nil == boundaryData ) {
		DDLogError(@"MultipartFormDataParser: Trying to parse multipart without specifying a valid boundary");
		assert(false);
        return NO;
    }
		...
		if( checkForContentEnd ) {
			if(	pendingData.length >= 2 ) {
				if( *(uint16_t*)(pendingData.bytes + offset) == 0x2D2D ) {
					...
					DDLogVerbose(@"MultipartFormDataParser: End of multipart message");
					waitingForCRLF = YES;
					reachedEpilogue = YES;
					offset+= 2;
				}
	...
	while( true ) {
		...
		if( reachedEpilogue ) {
			...
			if( [delegate respondsToSelector:@selector(multipartFormDataParser:processEpilogueData:)] ) {
				...
				[delegate multipartFormDataParser:self processEpilogueData: epilogueData];
			}
			return YES;
		}
		...
		if( nil == currentHeader ) {
			...
			if( -1 == headerEnd ) {
				...
				NSData * headerData = [NSData dataWithBytesNoCopy: (char*) workingData.bytes + offset length:headerEnd + 4 - offset freeWhenDone:NO];
				...
				if( nil == currentHeader ) {
					...
					DDLogError(@"MultipartFormDataParser: MultipartFormDataParser: wrong input format, coulnd't get a valid header");
					return NO;
				}
                if( [delegate respondsToSelector:@selector(multipartFormDataParser:processStartOfPartWithHeader:)] ) {
                    [delegate multipartFormDataParser:self processStartOfPartWithHeader:currentHeader];
                }

				DDLogVerbose(@"MultipartFormDataParser: MultipartFormDataParser: Retrieved part header.");
			}
		...
		if( contentEnd == -1 ) {
			...
			if( [delegate respondsToSelector:@selector(multipartFormDataParser:processContent:WithHeader:)] ) {
				DDLogVerbose(@"MultipartFormDataParser: Processed %d bytes of body",sizeToPass);

				[delegate multipartFormDataParser:self processContent: decodedData WithHeader:currentHeader];
			}
			...
			if( [delegate respondsToSelector:@selector(multipartFormDataParser:processContent:WithHeader:)] ) {
				[delegate multipartFormDataParser:self processContent:[NSData dataWithBytesNoCopy:(char*) workingData.bytes + offset length:contentEnd - offset freeWhenDone:NO] WithHeader:currentHeader];
			}

			if( [delegate respondsToSelector:@selector(multipartFormDataParser:processEndOfPartWithHeader:)] ){
				[delegate multipartFormDataParser:self processEndOfPartWithHeader:currentHeader];
				DDLogVerbose(@"MultipartFormDataParser: End of body part");
			}
			currentHeader = nil;
...
- (int) offsetTillNewlineSinceOffset:(int) offset inData:(NSData*) data {
	...
	while ( *(uint16_t*)(bytes + offset) != 0x0A0D ) {
		...
		if( !isspace(*(bytes+offset)) ) {
			DDLogWarn(@"MultipartFormDataParser: Warning, non-whitespace character '%c' between boundary bytes and CRLF in boundary line",*(bytes+offset) );
		}
		if( !isspace(*(bytes+offset+1)) ) {
			DDLogWarn(@"MultipartFormDataParser: Warning, non-whitespace character '%c' between boundary bytes and CRLF in boundary line",*(bytes+offset+1) );
		}
#endif
		offset++;
		if( offset >= length ) {
...
- (int) processPreamble:(NSData*) data {
	...
	if( offset == dataLength ) {
		...
		if( sizeToProcess > 0) {
			if( [delegate respondsToSelector:@selector(multipartFormDataParser:processPreambleData:)] ) {
				NSData* preambleData = [NSData dataWithBytesNoCopy: (char*) data.bytes length: data.length - offset - boundaryLength freeWhenDone:NO];
				[delegate multipartFormDataParser:self processPreambleData:preambleData];
				DDLogVerbose(@"MultipartFormDataParser: processed preamble");
			}
		...
		if ( offset && [delegate respondsToSelector:@selector(multipartFormDataParser:processPreambleData:)] ) {
...
//	http://tools.ietf.org/html/rfc2045#section-6.7
	...
	while( count < length ) {
		if( bytes[count] == '=' ) {
			...
			if( length < 3 ) {
				DDLogWarn(@"MultipartFormDataParser: warning, trailing '=' in quoted printable data");
			}
			// soft newline
			if( bytes[0] == '\r' ) {
				bytes += 1;
		...
		if( (unsigned char) bytes[count] > 126 ) {
			DDLogWarn(@"MultipartFormDataParser: Warning, character with code above 126 appears in quoted printable encoded data");
		}
#endif
